...
def get_model(exp_model, hist_len, pred_len, in_dim, city_num, batch_size, device,
    ...
    if exp_model == 'MLP':
        ...
        wind_channel_idx = (metero_use.index('u_component_of_wind+950'), metero_use.index('v_component_of_wind+950'))
        return ProbGRUModel2(
            hist_len, pred_len, in_dim, city_num, batch_size, device,
            graph.edge_index, graph.edge_attr, wind_mean, wind_std,
            station_coords=coords, station_elevation=altitude, wind_channel_idx=wind_channel_idx,
            hidden_dim=config['experiments'].get('gru_hidden_dim', 64),
            latent_dim=config['experiments'].get('gru_latent_dim', 16),
        ...
        metero_use = config['experiments']['metero_use']
        pbl_channel_idx = metero_use.index('boundary_layer_height') if 'boundary_layer_height' in metero_use else None
