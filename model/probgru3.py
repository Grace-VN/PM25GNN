...
class AdvectivePhysicsAwareSpatialAttention(nn.Module):
    ...
    meant to emerge from chaining this single-hop bias across decode steps
    (see ProbGRUModel6.decode_spatial_mix / spatial_mix_mode='per_step'
    below) using each step's own future wind reading, rather than from one
    closed-form multi-step displacement - a single cumulative displacement
    can't express a wind direction that shifts mid-window, and can't route
...
class ProbGRUModel3(nn.Module):
    """
    ProbGRUModel5 + wind-advected effective distance (see
    AdvectivePhysicsAwareSpatialAttention above). Everything else - the
    VAE bottleneck, decoder, spatial_mix_mode fork, decode_spatial_mix
    parity with PM25_GNN's per-decode-step message passing - is unchanged
    from probgru5.py, so results are directly comparable to v3/v4/v5
    ablations through the same trainer.

    spatial_mix_mode:
      - 'bottleneck': plain fused nn.GRU encodes each node's history
        independently; ONE advection-aware mixing pass happens at the end,
        using only the final observed wind reading.
      - 'per_step': encoder unrolled with a GRUCell; wind direction AND
        speed are recomputed at every history timestep, so d_eff is
        re-derived every step and the MIXED hidden state feeds back into
        the recurrence - this is what lets single-hop advection compound
        into effectively multi-step, multi-hop propagation over time,
        rather than needing a closed-form multi-step displacement formula.

    decode_spatial_mix (default False): also re-run advection-aware mixing
    at every decode step using that step's known future wind, matching
    PM25_GNN's GraphGNN which re-does wind-weighted message passing at
    every decode iteration. Reuses spatial_attn/mix_gate - no new params.
    """
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device,
        ...
        # kept for signature parity across benchmark models; spatial mixing
        # here uses station_coords/elevation/wind directly instead
    ...
    def _wind_vector_at(self, feature_slice, t, B, N):
        """feature_slice: [B, T, N, feature_dim] -> (wind_dir, wind_speed_ms) at
        ...
        wind_u = feature_slice[:, t, :, u_idx]
        wind_v = feature_slice[:, t, :, v_idx]
    ...
    def _encode_bottleneck(self, x, feature_hist, B, N):
        """Fused GRU, single advection-aware mixing pass at the end using
        the last observed history step's wind."""
    ...
    def _encode_per_step(self, x, feature_hist, B, N):
        """Unroll the encoder manually; at every timestep recompute wind
        direction AND speed, re-derive d_eff, mix in advection-aware
        context, and feed the MIXED hidden state back into the recurrence
        so single-hop advection compounds over the history window."""
        T = x.shape[1]
    ...
    def forward(self, pm25_hist, feature):
        ...
        for t in range(self.pred_len):
            ...
            if self.decode_spatial_mix:
                # PM25_GNN parity: recompute wind (direction AND speed) from
                # THIS step's future reading, re-derive d_eff, and let
                # stations exchange information again - instead of leaving
                # the decoder per-node-independent for the whole horizon.
                # Reuses spatial_attn/mix_gate - no new parameters.
                h_dec_grid = h_dec.reshape(B, N, self.hidden_dim)
                wind_dir_t, wind_speed_t = self._wind_vector_at(feature_future, t, B, N)
                context_t = self.spatial_attn(h_dec_grid, wind_dir_t, wind_speed_t)
