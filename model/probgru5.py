...
class ProbGRUModel5(nn.Module):
    ...
    decode_spatial_mix (default False, matches original behaviour) adds
    spatial mixing INSIDE the decoder loop as well. Without it, the decoder
    is per-node-independent for the entire forecast horizon: once encoding
    finishes, no information crosses between stations again, even though
    the physical premise (wind-driven transport) says it should - this is
    the gap relative to PM25_GNN's GraphGNN, which re-runs wind-weighted
    message passing at every decode step using that step's future wind
    reading. When True, this model does the same: at every decode step t,
    wind direction is recomputed from feature_future[:, t], spatial_attn is
    applied to the current decoder hidden state, and the mixed state is fed
    back into the GRUCell for the next step - reusing the same spatial_attn
    and mix_gate modules as the encoder, so no extra parameters are added
    ...
    def forward(self, pm25_hist, feature):
        ...
        for t in range(self.pred_len):
            ...
            if self.decode_spatial_mix:
                # PM25_GNN parity: recompute wind from THIS step's future
                # reading and let stations exchange information again,
                # instead of leaving the decoder per-node-independent for
                # the whole horizon. Reuses the same spatial_attn/mix_gate
                # weights as the encoder - no new parameters just to close
                # this gap.
                h_dec_grid = h_dec.reshape(B, N, self.hidden_dim)
                wind_dir_t = self._wind_dir_at(feature_future, t, B, N)
                context_t = self.spatial_attn(h_dec_grid, wind_dir_t)
